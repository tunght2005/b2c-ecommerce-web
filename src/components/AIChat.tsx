      ...
      try {
        ...
        if (history.length > 0) {
          ...
          history.slice().reverse().forEach((item, idx) => {
            const ts = new Date(item.createdAt || item.created_at || Date.now()).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const replyText: string =
        res?.reply || res?.data?.reply || res?.message || res?.data?.message ||
      ...
      >
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex gap-3 max-w-[85%] ${msg.role === 'user' ? 'ml-auto flex-row-reverse' : 'mr-auto'}`}>
              ...
              <div className={`flex flex-col gap-1 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`px-4 py-2.5 text-sm rounded-2xl whitespace-pre-wrap ${
                  msg.role === 'user'
        ...
        {messages.length <= 1 && (
          <div className="px-4 py-2 bg-gray-50 flex gap-2 overflow-x-auto no-scrollbar border-t border-gray-100">
            ...
            <button onClick={() => handleQuickReply('Điện thoại nào tốt nhất dưới 10 triệu?')} className="whitespace-nowrap px-3 py-1.5 bg-white border border-red-200 text-red-600 text-xs font-medium rounded-full hover:bg-red-50 transition">
              📱 Gợi ý điện thoại
            </button>
            <button onClick={() => handleQuickReply('Laptop gaming giá tốt')} className="whitespace-nowrap px-3 py-1.5 bg-white border border-red-200 text-red-600 text-xs font-medium rounded-full hover:bg-red-50 transition">
              💻 Laptop gaming
            </button>
          </div>
        )}
