...
export default function ProfileNotifications() {
  ...
  const getIcon = (type?: string) => {
    switch (type) {
      ...
      case 'promotion': return <Tag className="text-pink-500" size={20} />
    ...
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 min-h-full">
        ...
        {isLoading ? (
          ...
          notifications.map((noti) => (
            ...
            >
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className={`font-bold transition-colors ${
                      noti.is_read ? 'text-gray-700' : 'text-gray-900'
                    }`}>
                      {noti.title}
                    </h3>
                  ...
                  <p className={`text-sm mt-1 leading-relaxed ${
                    noti.is_read ? 'text-gray-500' : 'text-gray-700'
                  }`}>
                    {noti.content}
                  </p>
              ...
              {!noti.is_read && (
                <div className="absolute top-5 right-5 w-2 h-2 bg-red-600 rounded-full" />
              )}
            </div>
          ))
        )}
      ...
      <div className="mt-8 p-6 bg-gradient-to-br from-gray-900 to-gray-800 rounded-3xl text-white overflow-hidden relative">
        <div className="relative z-10">
          ...
          <p className="text-gray-300 text-sm opacity-90">Bật thông báo trên trình duyệt để không bỏ lỡ các đợt Flash Sale cực sốc từ SevenStore.</p>
        </div>
