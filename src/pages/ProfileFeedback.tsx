...
import {
  ...
  X, Loader2, HelpCircle, Search, Package, ShoppingBag
} from 'lucide-react';
...
export default function ProfileFeedback() {
  ...
  const fetchDropdownItems = async (tab: 'product' | 'order') => {
    ...
    try {
      if (tab === 'product') {
        ...
        setDropdownItems(list.map((p: any) => ({
          _id: p._id || p.id,
          name: p.name,
        ...
        const list: any[] = Array.isArray(res) ? res : (res?.data || []);
        setDropdownItems(list.map((o: any) => ({
          _id: o._id || o.id,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = activeTab === 'product'
        ? { product_id: formData.ref_id, title: formData.title, content: formData.content, priority: formData.priority }
        : { order_id: formData.ref_id, title: formData.title, content: formData.content, priority: formData.priority };
      await fetchClient('/feedback', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
  ...
  const getStatusBadge = (status: Feedback['status']) => {
    switch (status) {
      case 'open': return <span className="px-2.5 py-1 rounded-full bg-blue-100 text-blue-700 text-xs font-semibold flex items-center gap-1"><AlertCircle size={12} /> Chờ phản hồi</span>;
      case 'in_progress': return <span className="px-2.5 py-1 rounded-full bg-orange-100 text-orange-700 text-xs font-semibold flex items-center gap-1"><Clock size={12} /> Đang xử lý</span>;
      case 'resolved':
      case 'closed': return <span className="px-2.5 py-1 rounded-full bg-green-100 text-green-700 text-xs font-semibold flex items-center gap-1"><CheckCircle2 size={12} /> Đã trả lời</span>;
      default: return <span className="px-2.5 py-1 rounded-full bg-gray-100 text-gray-600 text-xs font-semibold">Không rõ</span>;
    }
  };
  ...
  // Lọc theo Tab
  const tabFeedbacks = feedbacks.filter(fb =>
    activeTab === 'product' ? !!fb.product_id : !!fb.order_id
  );
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {tabFeedbacks.map((fb) => {
              ...
              return (
                <div key={fb._id} className="border rounded-2xl p-5 hover:border-gray-300 transition-colors bg-gray-50/50">
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    {getStatusBadge(fb.status || 'open')}
                    <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider border ${pri.cls}`}>
                      {pri.label}
                    </span>
                    ...
                    <div className="flex items-center gap-3 mb-3 bg-white p-2 border border-gray-100 rounded-xl max-w-sm">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold text-gray-800">
                          #{(typeof fb.order_id === 'object' ? fb.order_id._id : String(fb.order_id)).substring(0, 8).toUpperCase()}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  <p className="text-gray-600 text-sm bg-white p-3 rounded-xl border border-gray-100 mb-3">{fb.content}</p>
          ...
          <div className="text-center py-20 flex flex-col items-center">
            <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-5 ${activeTab === 'product' ? 'bg-blue-50 text-blue-300' : 'bg-red-50 text-red-300'}`}>
            ...
            <button onClick={handleOpenModal} className="text-red-500 font-semibold hover:underline flex items-center gap-1">
              <Plus size={16} />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="border-b px-6 py-4 flex items-center justify-between bg-gray-50/50 flex-shrink-0">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-700 hover:bg-gray-100 p-2 rounded-full transition">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto flex-1">
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  {selectedItem && (
                    <div className="flex items-center gap-3 mb-2 bg-red-50 border border-red-200 rounded-xl px-3 py-2">
                      ...
                      <span className="text-sm font-semibold text-red-700 flex-1 line-clamp-1">{selectedItem.name}</span>
                      <button type="button" onClick={() => setFormData({ ...formData, ref_id: '' })} className="text-red-400 hover:text-red-600">
                        <X size={14} />
                      </button>
                    </div>
                  )}
                          ...
                          {filteredItems.length === 0 ? (
                            ...
                            >
                              {activeTab === 'product' && item.thumbnail ? (
                                <img src={item.thumbnail} alt="" className="w-9 h-9 rounded object-cover flex-shrink-0" />
                              ) : (
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    maxLength={120}
                    placeholder={activeTab === 'product' ? 'VD: Có chơi được game không? Có hỗ trợ 5G không?' : 'VD: Giao hàng quá trễ, Hàng bị hư hỏng...'}
              ...
              <div className="mt-6 flex justify-end gap-3 pt-4 border-t">
                ...
                >
                  {isSubmitting ? <Loader2 className="animate-spin" size={20} /> : (activeTab === 'product' ? 'Gửi câu hỏi' : 'Gửi khiếu nại')}
                </button>
              </div>
            </form>
          </div>
