...
import { Star, Loader2, Trash2, Edit, AlertCircle, ChevronDown, CheckCircle2, UserCircle2, ExternalLink } from 'lucide-react';
    ...
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 min-h-screen">
      ...
      {reviews.length === 0 ? (
        <div className="text-center py-20 flex flex-col items-center">
          ...
          <p className="text-gray-500 mb-6 max-w-sm">Bạn chưa viết bất kỳ đánh giá nào. Cùng viết đánh giá để chia sẻ trải nghiệm với người khác nhé!</p>
          <Link to="/profile/orders" className="text-red-600 font-bold bg-red-50 px-6 py-3 rounded-2xl hover:bg-red-100 transition">
        ...
        <div className="space-y-6">
          {reviews.map((review) => {
            ...
            return (
              <div key={review._id} className="border border-gray-100 rounded-3xl p-5 hover:border-red-100 hover:shadow-lg hover:shadow-red-50/50 transition duration-300">
                ...
                <div className="flex items-center gap-4 bg-gray-50/50 p-3 rounded-2xl mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-gray-900 line-clamp-1">{product?.name || 'Sản phẩm tiêu chuẩn'}</h4>
                    <Link to={`/products/${product?._id || ''}`} className="text-xs text-red-500 hover:text-red-600 font-medium inline-flex items-center gap-1 mt-0.5">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-gray-700 text-sm md:text-base leading-relaxed whitespace-pre-wrap">{review.content}</p>

                    {/* Review Images */}
                    {review.images && review.images.length > 0 && (
                      <div className="flex flex-wrap gap-2 mt-4">
                        {review.images.map((img, i) => (
                          <div key={i} className="w-20 h-20 rounded-2xl overflow-hidden border border-gray-100 hover:scale-105 transition cursor-pointer">
