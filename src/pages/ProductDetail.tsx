...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        setLoading(true)
        // Gọi song song dữ liệu chính sản phẩm
  ...
  useEffect(() => {
    ...
    const fetchBestPromotion = async () => {
      try {
        const res: any = await fetchClient(`/promotions/best/${selectedVariantId}`).catch(() => null)
  ...
  const originalPriceNumber = Number(rawPrice) || 0
  const promoDiscountValue = Number(bestPromo?.real_discount || 0)
  const promoFinalPriceNumber = Number(bestPromo?.final_price || 0)
  const effectivePriceNumber = bestPromo
    ? Math.max(0, promoFinalPriceNumber || originalPriceNumber - promoDiscountValue)
    : originalPriceNumber

  const finalPrice = formatCurrency(effectivePriceNumber)

  const rawOldPrice = selectedVariant?.old_price || selectedVariant?.oldPrice || product.oldPrice || originalPriceNumber
  const oldPriceNumber = Number(rawOldPrice) || 0
  const finalOldPrice = oldPriceNumber > effectivePriceNumber ? formatCurrency(oldPriceNumber) : null

  const finalDiscount =
    oldPriceNumber > 0 && effectivePriceNumber >= 0
      ? Math.max(0, Math.round(100 - (effectivePriceNumber / oldPriceNumber) * 100))
      : 0

  const specificationEntries =
    product?.specification && typeof product.specification === 'object'
      ? Object.entries(product.specification as Record<string, unknown>)
      : []

  const promotionPolicyLines = bestPromo
    ? [
        bestPromo?.discount_type === 'percent'
          ? `Giảm ${Number(bestPromo?.discount_value || 0)}% trực tiếp trên phiên bản đã chọn`
          : `Giảm ${formatCurrency(Number(bestPromo?.discount_value || 0))} trực tiếp trên phiên bản đã chọn`,
        bestPromo?.start_date ? `Bắt đầu: ${new Date(bestPromo.start_date).toLocaleString('vi-VN')}` : null,
        bestPromo?.end_date ? `Kết thúc: ${new Date(bestPromo.end_date).toLocaleString('vi-VN')}` : null,
        bestPromo?.type === 'flash_sale'
          ? 'Áp dụng theo số lượng flash sale còn lại'
          : 'Áp dụng theo chương trình khuyến mãi hiện hành'
      ].filter(Boolean)
    : []
  ...
  return (
    <div className='bg-gray-50 min-h-screen pb-6 sm:pb-10'>
      <Seo
        ...
        description={
          product?.description ||
          `Khám phá thông tin, giá bán, khuyến mãi và đánh giá của ${product?.name || 'sản phẩm'} tại 7Store.`
        }
        keywords={`${product?.name || 'sản phẩm'}, 7Store, khuyến mãi, mua sắm`}
        canonicalPath={`/product/${id || ''}`}
        image={selectedImage || resolveImageUrl(product?.thumbnail || product?.image) || undefined}
      />
        ...
        <div className='grid lg:grid-cols-[1.1fr_0.9fr] gap-4 sm:gap-6 mt-4 sm:mt-6'>
          ...
          <div className='space-y-4 sm:space-y-6'>
            ...
            <div className='bg-white rounded-2xl sm:rounded-3xl p-3 sm:p-5 shadow-sm border border-gray-100 relative'>
              ...
              {bestPromo && (
                <div className='absolute top-3 left-3 sm:top-5 sm:left-5 z-10 bg-linear-to-r from-red-600 to-orange-500 text-white px-3 py-1 sm:px-4 sm:py-1.5 rounded-full shadow-lg flex items-center gap-1.5 animate-bounce'>
              ...
              <img
                src={selectedImage}
                alt={product.name}
                className='w-full h-62.5 sm:h-87.5 lg:h-105 object-contain rounded-xl sm:rounded-2xl bg-gray-50 transition-all duration-300'
              />
            ...
            <div className='bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm border border-gray-100'>
              ...
              <div className='text-sm sm:text-base text-gray-700 leading-relaxed sm:leading-8 html-content'>
                {product.description || product.content ? (
                  ...
                  <p>Sản phẩm chính hãng từ 7Store. Thông số chi tiết được cập nhật bên dưới.</p>
                )}
              </div>

              {specificationEntries.length > 0 && (
                <div className='mt-5 border-t border-gray-100 pt-4'>
                  <h4 className='text-sm sm:text-base font-bold text-gray-900 mb-3'>Thông số kỹ thuật</h4>
                  <div className='grid grid-cols-1 sm:grid-cols-2 gap-2'>
                    {specificationEntries.map(([key, value]) => (
                      <div key={key} className='rounded-xl bg-gray-50 px-3 py-2 text-sm'>
          ...
          <div className='space-y-4 sm:space-y-6'>
            <div className='bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm border border-gray-100 lg:sticky lg:top-25'>
              {/* Box Giá tiền */}
              ...
              {/* Best Promo Gợi ý */}
              {bestPromo && (
                <div className='mt-4 bg-linear-to-r from-red-50 to-orange-50 border border-red-200 rounded-2xl p-3 flex items-center gap-3 shadow-sm'>
                  ...
                  <div>
                    <p className='text-sm font-bold text-red-700 line-clamp-1'>Ưu đãi áp dụng: {bestPromo.name}</p>
                    <p className='text-xs text-red-600/80 mt-0.5'>
                      Giảm ngay{' '}
                      <strong className='bg-red-100 px-1.5 py-0.5 rounded'>
                        {bestPromo.discount_type === 'percent'
                          ? `${Number(bestPromo.discount_value || 0)}%`
                          : formatCurrency(Number(bestPromo.discount_value || 0))}
                      </strong>{' '}
                      cho phiên bản đang chọn
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className='mt-6 sm:mt-8 space-y-3 sm:space-y-4 border-t pt-5 sm:pt-6'>
                {promotionPolicyLines.map((line, idx) => (
                  <div key={`${line}-${idx}`} className='flex items-center gap-3 text-sm sm:text-base text-gray-700'>
                    <Tag className='text-red-500 shrink-0' size={18} />
                    {line}
                  </div>
                ))}
                <div className='flex items-center gap-3 text-sm sm:text-base text-gray-700'>
                  <Truck className='text-red-500 shrink-0' size={18} />
                ...
                <div className='flex items-center gap-3 text-sm sm:text-base text-gray-700'>
                  <RotateCcw className='text-blue-500 shrink-0' size={18} />
                  Đổi trả miễn phí trong 30 ngày
                </div>
                <div className='flex items-center gap-3 text-sm sm:text-base text-gray-700'>
                  <Gift className='text-pink-500 shrink-0' size={18} />
                  Tặng kèm ưu đãi hấp dẫn
                </div>
                <div className='flex items-center gap-3 text-sm sm:text-base text-gray-700'>
                  <CreditCard className='text-purple-500 shrink-0' size={18} />
                  Hỗ trợ trả góp 0%
                </div>
              </div>
            </div>
