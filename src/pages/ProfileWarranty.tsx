...
export default function ProfileWarranty() {
  ...
  const handleClaimSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchWarranties();
      alert('Yêu cầu bảo hành đã được gửi thành công. Chúng tôi sẽ sớm liên hệ lại với bạn.');
    } catch (error: any) {
    ...
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Danh sách các thiết bị bạn đã mua và thông tin bảo hành tương ứng.</p>
        </div>
      </div>
      ...
      {warranties.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {warranties.map((w) => {
            ...
            return (
              <div key={w._id} className="border border-gray-100 rounded-2xl md:px-4 md:py-3 p-4 flex flex-col md:grid md:grid-cols-12 md:gap-4 items-center bg-gray-50/50 hover:bg-white hover:border-red-100 transition-colors">
                
                {/* Mobile view top part */}
                <div className="w-full flex justify-between md:hidden mb-2">
                  <span className="text-xs text-gray-400 font-medium">#{w._id.substring(w._id.length - 8).toUpperCase()}</span>
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                      isActive ? 'bg-green-50 text-green-600 border border-green-100' : 
                      isExpired ? 'bg-gray-100 text-gray-500 border border-gray-200' : 'bg-yellow-50 text-yellow-600 border border-yellow-100'
                    }`}>
                ...
                <div className="col-span-5 w-full">
                  <p className="font-semibold text-gray-800 break-words line-clamp-1">{extractItemLabel(w.order_item_id)}</p>
                  <p className="text-xs text-gray-400 mt-0.5">Ngày kích hoạt: {startDate ? new Date(startDate).toLocaleDateString('vi-VN') : 'Không rõ'}</p>
                </div>
                ...
                <div className="col-span-2 w-full md:text-center mt-3 md:mt-0 flex items-center justify-between md:block">
                  ...
                  <span className="text-sm font-medium text-gray-700 flex items-center md:justify-center gap-1.5"><Calendar size={14} className="text-gray-400"/> {w.warranty_period} tháng</span>
                </div>
                ...
                <div className="col-span-2 w-full md:text-center mt-2 md:mt-0 flex items-center justify-between md:block">
                   ...
                   <span className={`text-sm font-bold ${isExpired ? 'text-gray-400' : 'text-red-500'}`}>{daysLeft} ngày</span>
                </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl relative">
            <div className="p-6">
              ...
              <p className="text-sm text-gray-500 mb-6">
                ...
                <strong className="text-gray-800 ml-1">#{selectedWarranty._id.substring(selectedWarranty._id.length - 8).toUpperCase()}</strong>.
              </p>
              
              <form onSubmit={handleClaimSubmit}>
                <div className="mb-5">
                  <label className="block text-sm font-medium text-gray-700 mb-2">Mô tả sự cố <span className="text-red-500">*</span></label>
                  <textarea
                    required
