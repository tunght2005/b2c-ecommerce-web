...
function OrderDetail() {
  ...
  const getTimeline = () => {
    ...
    const currentIndex = steps.findIndex(s => s.key === order.status || (order.status === 'delivered' && s.key === 'completed'))
  ...
  return (
    <div className='max-w-7xl mx-auto px-4 py-8 bg-gray-50 min-h-screen'>
      ...
      <div className='flex items-center gap-4 mb-8'>
        ...
        <div>
          <h1 className='text-2xl sm:text-3xl font-black text-gray-900'>Chi tiết đơn hàng</h1>
          <p className="text-sm text-gray-500 font-medium">SevenStore • Mã đơn: #{order._id.substring(order._id.length - 8).toUpperCase()}</p>
        </div>
      </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-8'>
        ...
        <div className='lg:col-span-2 space-y-8'>
          ...
          <div className='bg-red-600 rounded-3xl p-8 text-white shadow-xl shadow-red-100 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden'>
             <div className="relative z-10">
                ...
                <p className='text-white/80 text-sm font-medium'>{statusDescriptions[order.status]}</p>
             </div>
             <div className="relative z-10 text-center md:text-right">
                <p className="text-white/60 text-xs uppercase font-bold tracking-widest mb-1">Tổng thanh toán</p>
                <p className="text-3xl font-black">{formatCurrency(order.final_price)}</p>
             </div>
          ...
          <div className='bg-white rounded-3xl p-8 shadow-sm border border-gray-100'>
            <h3 className='font-black text-gray-900 flex items-center gap-3 mb-8 text-lg'>
            ...
            <div className='relative pl-6 space-y-10'>
              ...
              {getTimeline().map((step, idx) => (
                <div key={idx} className={`relative z-10 flex gap-8 items-start ${!step.isDone ? 'opacity-30' : ''}`}>
                  <div className={`mt-1.5 w-5 h-5 rounded-full border-4 border-white shadow-md transition-all duration-500 ${
                    step.isCurrent ? 'bg-green-500 ring-8 ring-green-100 scale-125' : 
                    step.isDone ? 'bg-green-500' : 'bg-gray-200'
                  }`} />
                  <div>
                    <h4 className={`text-base font-bold transition-colors ${step.isCurrent ? 'text-green-600' : 'text-gray-800'}`}>
                      {step.label}
                    </h4>
                    {step.time && (
          ...
          <div className='bg-white rounded-3xl shadow-sm border border-gray-100 p-8'>
            ...
            <div className='space-y-6'>
              {order.items.map((item, index) => {
                ...
                return (
                  <div key={index} className='flex items-center gap-6 p-6 bg-gray-50/50 rounded-3xl border border-gray-100 hover:bg-gray-50 transition'>
                    <div className="relative">
                      <img 
                        src={item.image || 'https://via.placeholder.com/300x300?text=Sản+phẩm'} 
                        alt={item.name || "Sản phẩm"} 
                        className='w-24 h-24 object-cover rounded-2xl bg-white shadow-sm border border-gray-100' 
                      />
                      <div className="absolute -top-2 -right-2 bg-gray-900 text-white text-[10px] w-6 h-6 flex items-center justify-center rounded-full font-bold border-2 border-white">
                        {item.quantity}
                      </div>
                    </div>
                    <div className='flex-1'>
                      <h3 className='font-bold text-gray-900 text-lg'>{item.name || vInfo?.sku || "Sản phẩm SevenStore"}</h3>
                      <p className='text-gray-400 text-sm font-medium mt-1 uppercase tracking-tight'>SKU: {vInfo?.sku || 'N/A'}</p>
                      <div className="flex items-center justify-between mt-4">
                        <p className='text-gray-500 text-sm font-medium'>Đơn giá: {formatCurrency(item.price)}</p>
                        <p className='font-black text-red-600 text-lg'>{formatCurrency(item.price * item.quantity)}</p>
                      </div>
                    </div>
                  </div>
                )
        ...
        <div className='space-y-8'>
          <div className='bg-white rounded-3xl shadow-sm border border-gray-100 p-8'>
            ...
            <div className='space-y-6'>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className='text-xs text-gray-400 uppercase font-black'>Ngày lập hóa đơn</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className='text-xs text-gray-400 uppercase font-black'>Trạng thái thanh toán</p>
                  <p className={`font-bold ${order.payment_status === 'paid' ? 'text-green-600' : 'text-orange-600'}`}>
                    {order.payment_status === 'paid' ? 'Đã thanh toán' : 'Chờ thanh toán (VNPAY/COD)'}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className='bg-white rounded-3xl shadow-sm border border-gray-100 p-8 relative overflow-hidden'>
            ...
            <p className='text-gray-700 text-sm leading-relaxed font-medium whitespace-pre-wrap'>{shippingAddress}</p>
            <div className="mt-6 flex gap-2">
               <span className="px-3 py-1 bg-gray-900 text-white text-[10px] rounded-full font-bold uppercase">Nhà riêng</span>
            </div>
          </div>

          <div className='bg-white rounded-3xl shadow-sm border border-gray-100 p-8'>
            <h2 className='text-lg font-black text-gray-900 mb-6'>Giá trị đơn hàng</h2>
            <div className='space-y-4 font-medium'>
              ...
              <div className='border-t border-dashed border-gray-200 pt-6 mt-4 flex justify-between items-center'>
                <span className="text-gray-900 font-black uppercase text-xs">Tổng cộng</span>
                <span className='text-red-600 text-3xl font-black italic'>{formatCurrency(order.final_price)}</span>
              </div>
            </div>
          </div>
          ...
          <div className='space-y-4'>
            {order.status === 'completed' && (
              <button 
                className='w-full py-4 bg-gray-900 text-white rounded-2xl font-bold shadow-xl shadow-gray-200 hover:bg-black transition scale-100 active:scale-95'
              >
                Đánh giá sản phẩm
              </button>
            )}
